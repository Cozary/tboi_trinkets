...
public class BlueBaby extends CurioItemBase {

    public int ticksToExistGhost = 20;
    ...
    public void curioTick(String identifier, int index, LivingEntity livingEntity, ItemStack stack) {
        ...
        if (worldIn instanceof ServerWorld) {
            if (!worldIn.isRemote && --ticksToExistGhost <= 0) {
                WorldUtil.spawnBlueBaby(((ServerWorld) worldIn).getWorldServer(), pos);
                ticksToExistGhost = 200;
            }
        }
    }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent(TextFormatting.DARK_AQUA + "" + TextFormatting.ITALIC + "Imaginary Friend"));
    }
}